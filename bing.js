...
async function getVoiceSpeech(text) {
	try {
		const data = await getToken();
		const tokenData = await getVoiceToken(data);
		const xmlBody =
			"<speak version='1.0' xml:lang='vi-VN'><voice xml:lang='vi-VN' xml:gender='Male' name='vi-VN-An'><prosody rate='-20.00%'>Chào bạn tuấn B và K</prosody></voice></speak>";
		const config = {
			headers: {
